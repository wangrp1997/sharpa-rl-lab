...
parser.add_argument("--turn_steps", type=int, default=24)
...
def main(env_cfg: DirectRLEnvCfg, agent_cfg: dict):
    ...
    records = [{"event": "contacts", "env": env_id, "loads": loads0.tolist()}]
    ...
    records.append({"event": "result", "loads": loads1.tolist()})
    print(f"result loads={[round(float(v), 3) for v in loads1]}", flush=True)
